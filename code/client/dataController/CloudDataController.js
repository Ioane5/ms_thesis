export default class CloudDataController {
    ...
    constructor(publicKey, privateKey, baseUrl) {
        ...
        this.encodedPublicKey = encodeURIComponent(this.publicKey);
    }

    sync(callback) {
        get(this.baseUrl + '/messages/list/' + this.encodedPublicKey).then((response) => {
            let messageList = JSON.parse(response);
            console.log('Sync: Response:', messageList);
            messageList.forEach((messageId) => {
                this.downloadMessage(messageId, (message) => {
                    if (message) {
    ...
    deleteMessage(messageId, callback) {
        ...
        let url = this.baseUrl + '/messages/' + messageId + '?userId=' + this.encodedPublicKey;
        req.open('DELETE', url);
        req.onload = () => {
    ...
    messagePromise(messageId, publicMessage) {
        ...
        if (publicMessage) {
            ...
            url = this.baseUrl + '/messages/' + messageId + '?userId=' + this.encodedPublicKey;
        }
